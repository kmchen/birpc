...
func isExportedOrBuiltinType(t reflect.Type) bool {
	for t.Kind() == reflect.Ptr {
		// t.Kind() == ptr or struct
		t = t.Elem()
		//t.Elem() returns the struct of the pointer bidirpc.Reply
	}
...
func (p *Protocol) register(rcvr interface{}, name string, useName bool) error {
	...
	//rcvr typeOf  -> type : *bidirpc.Arith
	s.rcvr = reflect.ValueOf(rcvr)
	//rcvr valueOf  -> value : <*bidirpc.Arith Value>
	...
	// reflect.Indirect(s.rcvr) <bidirpc.Arith Value>
	// reflect.Indirect(s.rcvr).Type() bidirpc.Arith
	// reflect.Indirect(s.rcvr).Type().Name() Arith
...
func suitableMethods(typ reflect.Type, reportErr bool) map[string]*methodType {
	...
	for m := 0; m < typ.NumMethod(); m++ {
		...
		mname := method.Name
		//{Name:Add
		//PkgPath: Type:func(*bidirpc.Arith, bidirpc.Args, *bidirpc.Reply) error
		//Func:<func(*bidirpc.Arith, bidirpc.Args, *bidirpc.Reply) error Value>
		//Index:0}
		//method type : func(*bidirpc.Arith, bidirpc.Args, *bidirpc.Reply) error
		//method name : Add
...
func (s *service) call(server *Protocol, mtype *methodType, req *RepReq, argv, replyv reflect.Value) {
	...
	// function  <func(*bidirpc.Arith, bidirpc.Args, *bidirpc.Reply) error Value>

	// Invoke the method, providing a new value for the reply.
	returnValues := function.Call([]reflect.Value{s.rcvr, argv, replyv})
	//s.rcvr  <*bidirpc.Arith Value>
	//argv  <bidirpc.Args Value>
	//replyv  <*bidirpc.Reply Value>
...
ENDFOR:
	for !p.closing {
		...
		//&{Type:1 ServiceMethod:Arith.Add Seq:0 Error: next:<nil>}
		// Grab the request header.
		if err != nil {
			repReq = nil
		...
		case REQUEST:
			...
			// repReq.ServiceMethod : Arith.Add
			// dot = 5
			if dot < 0 {
			...
			//mtype.ReplyType = *bidirpc.Reply
			//mtype.ReplyType.Elem() = bidirpc.Reply
			replyv = reflect.New(mtype.ReplyType.Elem())
			//replyv is a reflect.Value <*bidirpc.Reply Value>
